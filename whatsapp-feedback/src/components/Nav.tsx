...
export function Nav({ active, transparent }: { active?: Page; transparent?: boolean }) {
  ...
  return (
    <>
      <nav className={`w-full px-6 lg:px-10 flex items-center justify-between sticky top-0 z-50 transition-colors ${transparent ? "bg-transparent" : "bg-surface-primary/90 backdrop-blur-sm"}`} style={{ minHeight: 80, paddingTop: "env(safe-area-inset-top)" }}>
        <a href="/">
