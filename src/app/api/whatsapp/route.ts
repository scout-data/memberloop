...
async function findMatchingEvents(phone: string, profile: Record<string, unknown>, userMessage: string): Promise<string> {
  try {
    ...
    // Only recommend events we can actually link to
    candidates = candidates.filter(e => e.details_url !== null);
    ...
    const lines = candidates.slice(0, 9).map((e, i) => {
      ...
      const slug = (e.details_url as string).replace("https://www.gigpig.uk/whats-on/", "");
      return `${i + 1}. ${e.title} at ${e.venue_name} — ${date} at ${time} [slug: ${slug}]`;
    }).join("\n");
